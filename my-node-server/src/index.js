...
const items = [];
for (let i = 0; i < 3; i++) {
  items.push(new VideoGameIdea({ id: `${i}`, text: `videoGameIdea ${i}`, date: new Date(Date.now() + i), version: 1 }));
}
...
router.get('/videoGameIdea/:id', async (ctx) => {
  ...
  if (item) {
    ...
    ctx.response.body = { issue: [{ warning: `videoGameIdea with id ${itemId} not found` }] };
...
router.post('/videoGameIdea', async (ctx) => {
  await createItem(ctx);
});

router.put('/videoGameIdea/:id', async (ctx) => {
  ...
  if (index === -1) {
    ctx.response.body = { issue: [{ error: `videoGameIdea with id ${id} not found` }] };
    ctx.response.status = 400; // BAD REQUEST
    return;
  }
...
router.del('/videoGameIdea/:id', ctx => {
  const id = ctx.params.id;
...
setInterval(() => {
  ...
  const item = new VideoGameIdea({ id: lastId, text: `videoGameIdea ${lastId}`, date: lastUpdated, version: 1 });
  items.push(item);
  console.log(`New videoGameIdea: ${item.text}`);
  broadcast({ event: 'created', payload: { item } });
}, 15000);
